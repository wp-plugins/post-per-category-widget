 ...
 $args=array('orderby'=>'name','type'=>'post','exclude'=>$cat,'taxonomy'=> 'category');
                       ...
                       $out_cat=$categories->name."<br>";
                           ...
                           //$out_posts.='<li><a href="'.get_category_link($categories->cat_ID).'">'.$categories->name.'</a></li>';
                            ...
                            $thumb= wp_get_attachment_image(get_post_thumbnail_id($post->ID), array(46,46),true);
	                    $out_posts.=<<<HTML
                            <li>
                            <h5>{$out_cat}</h5>
                            <a href="{$p_link}" title="$p_title"><div style="float:left;">{$thumb}</div>{$p_title}</a>
                            </li>
HTML;
